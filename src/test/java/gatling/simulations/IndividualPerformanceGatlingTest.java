...
/**
 * Performance test for the IndividualPerformance entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.7.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class IndividualPerformanceGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all individualPerformances")
                    .get("/api/individual-performances")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new individualPerformance")
                        .post("/api/individual-performances")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"completedTasks\": 0" + ", \"velocity\": 0" + ", \"storyPointsCompleted\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_individualPerformance_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created individualPerformance").get("${new_individualPerformance_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created individualPerformance")
                        .delete("${new_individualPerformance_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the IndividualPerformance entity").exec(scn);

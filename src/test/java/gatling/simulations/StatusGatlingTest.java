 ...
 */
public class StatusGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all statuses").get("/api/statuses").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new status")
                        .post("/api/statuses")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created status").get("${new_status_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created status").delete("${new_status_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Status entity").exec(scn);

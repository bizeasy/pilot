...
public class SprintTaskGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all sprintTasks").get("/api/sprint-tasks").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new sprintTask")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"thruTime\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"assignedTime\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"duration\": null" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_sprintTask_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created sprintTask").get("${new_sprintTask_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created sprintTask").delete("${new_sprintTask_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the SprintTask entity").exec(scn);

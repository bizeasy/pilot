 ...
 */
public class TaskHistoryGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all taskHistories").get("/api/task-histories").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new taskHistory")
                        .post("/api/task-histories")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"type\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created taskHistory").get("${new_taskHistory_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created taskHistory").delete("${new_taskHistory_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TaskHistory entity").exec(scn);

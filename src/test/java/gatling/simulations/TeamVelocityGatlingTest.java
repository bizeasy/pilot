...
public class TeamVelocityGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all teamVelocities").get("/api/team-velocities").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new teamVelocity")
                        .post("/api/team-velocities")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"sprintVelocity\": 0" + ", \"averageVelocity\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_teamVelocity_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created teamVelocity").get("${new_teamVelocity_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created teamVelocity").delete("${new_teamVelocity_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TeamVelocity entity").exec(scn);

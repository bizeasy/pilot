 ...
 */
public class PartyGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all parties").get("/api/parties").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new party")
                        .post("/api/parties")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created party").get("${new_party_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created party").delete("${new_party_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Party entity").exec(scn);

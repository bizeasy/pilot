...
import statusCategoryResolve from './status-category-routing-resolve.service';

describe('StatusCategory routing resolve service', () => {
  let mockRouter: Router;
  let mockActivatedRouteSnapshot: ActivatedRouteSnapshot;
  let service: StatusCategoryService;
  let resultStatusCategory: IStatusCategory | null | undefined;
  ...
  describe('resolve', () => {
    it('should return IStatusCategory returned by find', () => {
      ...
      TestBed.runInInjectionContext(() => {
        statusCategoryResolve(mockActivatedRouteSnapshot).subscribe({
          next(result) {
    ...
    it('should return null if id is not provided', () => {
      ...
      expect(resultStatusCategory).toEqual(null);
    });

    it('should route to 404 page if data not found in server', () => {
      // GIVEN
      jest.spyOn(service, 'find').mockReturnValue(of(new HttpResponse<IStatusCategory>({ body: null })));
      mockActivatedRouteSnapshot.params = { id: 123 };

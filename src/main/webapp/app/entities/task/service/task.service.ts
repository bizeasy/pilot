...
type RestOf<T extends ITask | NewTask> = Omit<T, 'dueDate' | 'startTime' | 'pauseTime' | 'endTime'> & {
  dueDate?: string | null;
...
export class TaskService {
  ...
  protected convertDateFromClient<T extends ITask | NewTask | PartialUpdateTask>(task: T): RestOf<T> {
    return {
      ...
      startTime: task.startTime?.toJSON() ?? null,
      pauseTime: task.pauseTime?.toJSON() ?? null,
      endTime: task.endTime?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restTask: RestTask): ITask {
    return {
      ...
      startTime: restTask.startTime ? dayjs(restTask.startTime) : undefined,
      pauseTime: restTask.pauseTime ? dayjs(restTask.pauseTime) : undefined,
      endTime: restTask.endTime ? dayjs(restTask.endTime) : undefined,
    };
  }

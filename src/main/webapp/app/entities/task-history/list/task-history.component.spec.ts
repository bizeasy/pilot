...
import { sampleWithRequiredData } from '../task-history.test-samples';
...
describe('TaskHistory Management Component', () => {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.taskHistories?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });

  describe('trackId', () => {
    it('Should forward to taskHistoryService', () => {
      const entity = { id: 123 };
      jest.spyOn(service, 'getTaskHistoryIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getTaskHistoryIdentifier).toHaveBeenCalledWith(entity);
